    ...
    if(goods.length) {
        ...
        for(let i = 0; i < goods.length; i++) {
            ...
            <tr class ="align-middle">
                ...
                <td>${goods[i][3]}</td>
                <td><button class ="good_delete text-bg-danger" data-delete="${goods[i][0]}">&#10006; </button></td>
                <td><button class ="good_delete text-bg-primary" data-goods="${goods[i][0]}">&#10149; </button></td>
            </tr>
            `)
            if(goods[i][4] > 0) {
                ...
                <tr class ="align-middle">
                    ...
                    <td class ="price_count"><input data-goodid = "${goods[i][0]}" type = "text" value = "${goods[i][5]}" min = "0" max = "100"></td>
                    <td>${goods[i][6]}</td>
                    <td><button class ="good_delete text-bg-danger" data-delete="${goods[i][0]}">&#10006; </button></td>
...
document.querySelector('.list').addEventListener('click', function(e){
    if(!e.target.dataset.delete){ //data-delete
        return
    }
    Swal.fire({
        title: 'Внимание!',
...
document.querySelector('.list').addEventListener('click', function(e){
    if(!e.target.dataset.goods){ //data-goods
        return
    }
    let goods = JSON.parse(localStorage.getItem('goods'))
    for (let i = 0; i < goods.length; i++) {
...
document.querySelector('.cart').addEventListener('click', function(e){
    if(!e.target.dataset.delete){ //data-goods
        return
    }
    let goods = JSON.parse(localStorage.getItem('goods'))
    for (let i = 0; i < goods.length; i++) {
...
document.querySelector('.cart').addEventListener('input', function(e){
    if(!e.target.dataset.goodid){ //data-goodid
        return
    }
    let goods = JSON.parse(localStorage.getItem('goods'))
    for (let i = 0; i < goods.length; i++) {
